...
| avatar | hardened | ✅ | ⏳ | Display-1b (added fail-loud `coerce_size`; image alt / initials accessible-name / decorative aria-hidden; `bg-hue-initials`+white is the fixed AAA hue utility — correctly NOT "fixed"). 0a render test; app 0b/AAA CI pending |
| iframe | hardened | ✅ | ⏳ | Display-1b (required non-blank `title:` accessible name; loading clamp). 0a render test; app 0b/AAA CI pending |
| picture | hardened | ✅ | ⏳ | Display-1b (required base-img `alt`; `<source>` art-direction/format fallbacks; loading clamp — mirrors `image`). 0a render test; app 0b/AAA CI pending |
| device_mockup | hardened | ✅ | ⏳ | Display-1b (raw palette `bg-white`/`bg-red-400`… → semantic tokens; decorative notch/browser-chrome aria-hidden; fail-loud phone/browser/tablet variant). 0a render test; app 0b/AAA CI pending |
| map_area | hardened | ✅ | ⏳ | Display-1b (fail-loud alt-per-`<area href>` — was silent `alt=""`; img alt + usemap↔map-name match; legacy mechanism, documented). 0a render test; app 0b/AAA CI pending |
| timeline | hardened | ✅ | ⏳ | Display-1b (raw `bg-green-500`/`amber-500` → semantic signal dots; `<ol>`/`<li>` order + decorative aria-hidden dots; `<time datetime>`; fail-loud + destructive alias). 0a render test; app 0b/AAA CI pending |
| scroll_area | hardened | ✅ | ⏳ | Display-1b (WCAG 2.1.1: focusable region `tabindex=0`+`role=region`+mandatory name+`focus-ring`; `focusable:false` opt-out; fail-loud orientation + missing-name). 0a render test; app 0b/AAA CI pending |
