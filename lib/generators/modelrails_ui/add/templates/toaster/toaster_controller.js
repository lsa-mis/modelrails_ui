...
export default class extends Controller {
  ...
  #buildToast({ message = "", title = "", variant = "default", duration = 4000 }) {
    const borderCls = {
      ...
      destructive: "border-danger-border/40",
    ...
    div.className = [
      ...
      "bg-surface-raised px-4 py-3 shadow-lg text-text-heading",
    ...
    const bodyHtml = title
      ? `<p class="text-sm font-semibold leading-tight">${this.#esc(title)}</p>
         <p class="text-sm leading-snug text-text-muted mt-0.5">${this.#esc(message)}</p>`
    ...
    div.innerHTML = `
      ...
      <button type="button" aria-label="Dismiss"
        ...
        class="ml-auto -mr-1 -mt-0.5 shrink-0 inline-flex size-6 items-center justify-center
               rounded-md text-text-muted hover:text-text-heading hover:bg-surface-sunken
